...
The sc.conf is parsed using C<AppConfig> module and has a simple "variable =
value" form. The file may contain extra tabs and newlines for formatting.
...
=item * 0 - no debug (default value),

=item * 1 - print command line that caused error,

=item * 2 - print command line without execution.
...
Name of set with allowed IP's. Default value: pass
...
=item * C<ipmap> - stores IP's from one /16 network defined by C<network>
parameter (memory cheap and very fast).

=item * C<iphash> - stores IP's from arbitrary /16 networks.

=back

=item B<network>

Network for ipmap set.
Default value: 172.16.0.0/16
Possible values: any network with mask value between 16-31
...
For names other than FORWARD the new chain will be created and
...
Possible values: all database drivers supported by Perl DBI module. See output
of C<sc help> command for list of available drivers.
...
sc(8), App::Config(3), Sys::Syslog(3), syslog(3), tc(8), iptables(8),
ipset(8).
