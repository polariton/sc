...
addresses with the zero last octet due to limitations of tc(8) hashing filters
syntax.
...
Note, that B<policing> method only works with B<u32> filtering method.
B<hybrid> method uses shaping for download and policing for upload, and all
the rules are created on the B<in_if> interface.
This is suitable, when shaper performs NAT on the outbound interface.
