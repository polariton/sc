...
Warning: you can not use the IP's with zero last octet with this method due to
limitations of tc(8) syntax.
...
B<Hybrid> method uses shaping for download and policing for upload, and all rules
are created on the B<in_if> interface.
...
=item B<policer_burst>

Amount of bytes to buffer for every filter with policing rules.
Default value: 1500Kb.
This value is suitable for users with large bandwidths (up to 40mibit/s).
If the average bandwidths in your network are much lower, feel free to
decrease it.
