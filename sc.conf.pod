...
=item * ipmap - stores IP's from one /16 network defined by C<network>
parameter (memory cheap and very fast).

=item * iphash - stores IP's from arbitrary /16 networks.
...
of C<sc help> command for the list of available drivers.
